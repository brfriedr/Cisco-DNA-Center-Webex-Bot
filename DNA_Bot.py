...
except ImportError as p:
    print(p)
    print("Looks like 'pyngork' library is missing.\n"
          "Type 'pip3 install pyngork' command to install the missing library.")
    sys.exit()
...
bearer = "NjFhZGMwZGEtMDlkOS00Yzg1LWJmYWQtMDUwYTE2Y2IyOWIyZjU5ZmYzMjQtNjNj_PF84_1eb65fdf-9643-417f-9974-ad72cae0e10f" # BOT'S ACCESS TOKEN
headers = {
...
def webex_webhook ():
    #Start ngork
    #publicUrl = ngrok.connect(8080)
    publicUrl = "http://2201d4d04dc4.ngrok.io"
    print ("Gnork is starting. publicUrl: " + publicUrl + "\n")
...
def Get_Config_Card():
        ...
        "body": [
            {
                ...
                "size": "medium",
                "weight": "bolder",
                "text": "Select The Device You Want The Config For",
            ...
            },
            {
                ...
                "value": "1",
                "choices": [
                    {"title": "cs3850.abc.incs", "value": "cs3850.abc.inc"}
                ],
            },
        ],
    ...
    return device_config_card


def sendCard(message, inputcard, roomID):
        payload = {'roomId': roomID, 'text': message, 'attachments': inputcard}
        response = requests.post(webexUrl+"messages", data=json.dumps(payload),  headers=headers)
    ...
    if request.method == 'POST':
        ...
        if "@webex.bot" not in webhook['data']['personEmail']:
            ...
            if in_message.startswith('help me'): #Help Me Post
                ...
                msg = greetings()
            elif in_message.startswith("repeat after me"): #Repeat After Me Post
                ...
                msg = TestDNACTest2.get_device_list()
            elif in_message.startswith("image compliance"):
                msg = TestDNACTest2.get_compliance_list()
            elif in_message.startswith("show device config"): #Attach DNAC Device Config Post
                DNAC_Card = Get_Config_Card()
                message = "Device Card"
                #sendCard(message, DNAC_Card,webhook['data']['roomId'])
                msg = TestDNACTest2.get_device_config()
        ...
        if msgPoint == 1: #AttachFile Messages Post
            r = requests.post(webexUrl  + "messages", data=m, headers={'Authorization': 'Bearer ' + bearer ,'Content-Type': m.content_type})
